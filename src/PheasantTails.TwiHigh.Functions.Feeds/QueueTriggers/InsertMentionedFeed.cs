...
public class InsertMentionedFeed
{
    ...
    private readonly ILogger<InsertMentionedFeed> _logger;

    public InsertMentionedFeed(
        CosmosClient client,
        ILogger<InsertMentionedFeed> logger)
    {
        _client = client;
        _logger = logger;
    }
        ...
        [QueueTrigger(AZURE_STORAGE_FEED_MENTIONED_BY_USER_QUEUE_NAME, Connection = QUEUE_STORAGE_CONNECTION_STRINGS_ENV_NAME)] string myQueueItem)
    {
        try
        {
            ...
            catch (CosmosException ex)
            {
                ...
                {
                    _logger.TwiHighLogWarning(FUNCTION_NAME, "Target tweet is NOT found. ID: {0}, {1}", que.TargetTweetId, que.FeedByTweetId);
                    return;
                }
            ...
            _logger.TwiHighLogInformation(FUNCTION_NAME, "Target tweet is found. ID: {0}", targetTweet.Id);
            _logger.TwiHighLogInformation(FUNCTION_NAME, "{0} > {1}", targetTweet.UserDisplayId, targetTweet.Text);
            _logger.TwiHighLogInformation(FUNCTION_NAME, "Reply from tweet is found. ID: {0}", feedByTweet.Id);
            _logger.TwiHighLogInformation(FUNCTION_NAME, "{0} > {1}", feedByTweet.UserDisplayId, feedByTweet.Text);
            ...
            _logger.TwiHighLogInformation(FUNCTION_NAME, "Created Feed. Total RU: {0:0.00}", targetAndReplyFromTweets.RequestCharge + user.RequestCharge + result.RequestCharge);
        }
        catch (Exception ex)
        {
